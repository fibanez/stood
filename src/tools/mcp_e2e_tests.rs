...
impl E2EMockTransport {
        ...
        self.tool_handlers.insert(
            ...
            Box::new(|params| {
                ...
                let result = match operation {
                    ...
                    "summary" => format!("Text summary: {} words, {} characters", 
                                       text.split_whitespace().count(), text.len()),
    ...
    fn execute_tool(&self, tool_name: &str, params: &serde_json::Value) -> std::result::Result<Vec<Content>, StoodError> {
        match self.tool_handlers.get(tool_name) {
            Some(handler) => Ok(handler(params)),
            None => Err(StoodError::tool_error(format!("Tool '{}' not found", tool_name))),
        }
    }
}
...
impl E2EMockMCPClient {
    ...
    pub async fn session_info(&self) -> std::result::Result<(String, String, String, String), StoodError> {
        Ok((
...
async fn create_test_tool_registry() -> std::result::Result<(Arc<ToolRegistry>, MCPToolRegistry), StoodError> {
...
mod tests {
    ...
    async fn test_e2e_mock_client_tool_execution() {
        ...
        let result = client.call_tool("text_analyzer", text_params).await.unwrap();
    ...
    async fn test_mcp_tool_execution_through_registry() {
        ...
        let result = tool_registry.execute_tool(&tool_use.name, Some(tool_use.input.clone()), None).await;
