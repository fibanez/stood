...
When using MCP tools specifically, always mention that you used the MCP tool and include its complete response when asked.")
    ...
    match agent.execute(prompt1).await {
        Ok(result) => {
            ...
            let has_metadata_marker = result
                .response
                .contains("[This response came from the MCP server");
            ...
            if has_mcp_marker && has_search_marker && has_metadata_marker {
                ...
                if !has_mcp_marker {
                    println!("❌ Missing: 'MCP SERVER' marker");
                }
                if !has_search_marker {
                    println!("❌ Missing: '🔍' search marker");
                }
                if !has_metadata_marker {
                    println!("❌ Missing: '[This response came from the MCP server' metadata");
                }
                if !has_testing123 {
                    println!("❌ Missing: 'testing123' search term");
                }

                // Show what we got instead
                println!(
                    "🔍 Actual response preview (first 200 chars): {}",
    ...
    match agent.execute(prompt2).await {
        Ok(result) => {
            ...
            let has_metadata_marker = result
                .response
                .contains("[This timestamp came from the MCP server");
            let has_timestamp =
                result.response.contains("2025-") || result.response.contains("2024-");
            ...
            if has_mcp_marker && has_time_marker && has_metadata_marker {
                ...
                if !has_time_marker {
                    println!("❌ Missing: '⏰' time marker");
                }
                if !has_metadata_marker {
                    println!("❌ Missing: '[This timestamp came from the MCP server' metadata");
                }
                if !has_timestamp {
                    println!("❌ Missing: timestamp format");
                }
    ...
    match agent.execute(prompt3).await {
        Ok(result) => {
            ...
            let search_markers =
                result.response.contains("🔍") && result.response.contains("verification");
            ...
            if search_markers && time_markers && mcp_markers {
                ...
                if !search_markers {
                    println!("❌ Missing search markers");
                }
                if !time_markers {
                    println!("❌ Missing time markers");
                }
                if !mcp_markers {
                    println!("❌ Missing MCP SERVER markers");
                }
            }
        }
        Err(e) => {
