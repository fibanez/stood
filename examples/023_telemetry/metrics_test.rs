...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🧪 Testing Stood Telemetry with Multiple Tools - Span Hierarchy Validation (Nova Micro)");
    
    // Initialize telemetry for comprehensive tracing
    let telemetry_config = TelemetryConfig::from_env();
    println!("📊 Telemetry config: enabled={}, endpoint={:?}", 
    ...
    println!("✅ Agent created with 6 tools and comprehensive telemetry enabled (using Nova Micro)");
    ...
    println!("   • Complete span hierarchy: event_loop -> model_interaction -> model -> tool calls");
